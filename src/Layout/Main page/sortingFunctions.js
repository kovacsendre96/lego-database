    ...
    sortArrayByType.filter((data) => {
        ...
        iterateLegoData.forEach((lData) => {
            ...
            if (yearReleased === parseInt(filterData[key])) {
                ...
                iterateLegoData = iterateLegoData.filter((oldData) => oldData !== filterData);
            }
...
export const sortByType = (legoData, sortDirection, dataKey) => {
    ...
    switch (dataKey) {
        case 'year_released': sortResult = sortByKey(sortArrayByType, sortDirection, legoData, 'year_released');
            break;
        case 'name': sortResult = sortyByName(sortArrayByType, sortDirection, legoData);
            break;
        case 'number_of_pieces': sortResult = sortByKey(sortArrayByType, sortDirection, legoData, 'number_of_pieces');
            break;
        case 'max_price': sortResult = sortByKey(sortArrayByType, sortDirection, legoData, 'max_price');
            break;
    }
    return sortResult;
};
